...
export const sortBooks = (books, sortBy) => {
  switch (sortBy) {
    ...
    case SORT_FILTER.YEAR_ASC:
      return books.toSorted((book1, book2) =>
        book1.published.localeCompare(book2.published),
      );
    case SORT_FILTER.YEAR_DESC:
      return books.toSorted((book1, book2) =>
        book2.published.localeCompare(book1.published),
      );
...
export const filterBooks = (books, searchStr) => {
  const normalSearchStr = searchStr.toLowerCase().trim()
  return books.filter((book) => book.title.includes(normalSearchStr)|| book.author.includes(normalSearchStr));
};
